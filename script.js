let dataSet = [`1. What do you call a boomerang that won’t come     back?`,
  `A stick.`,
...
//JavaScript...
// const jokes = (() => {
//   document.body.childNodes[5].firstElementChild.innerHTML = '<b>Try another joke</b>'
//   let x1 = document.body.childNodes[3].childNodes[1]
//   let x2 = document.body.childNodes[3].childNodes[3]

//   let y = Math.floor(Math.random() * (dataSet.length - 1))

//   if (y % 2 == 0) {
//     x1.innerHTML = dataSet[y]
//     x2.innerHTML = dataSet[y + 1]
//   }
//   else {
//     x1.innerHTML = dataSet[y - 1]
//     x2.innerHTML = dataSet[y]
//   }
 //   if(dataSet[y].search("?") == -1){
 //      x1.innerHTML = dataSet[y]
 //      x2.innerHTML = dataSet[y+1]
 //    }
 //    else{
...
$(document).ready(() => {
  $('#btn').on('click', () => {
    $('#btn').html('<b>Try another joke</b>')
    
    let y = Math.floor(Math.random() * (dataSet.length - 1))

    if (y % 2 == 0) {
      $('#joke-qstn').html(dataSet[y])
      $('#joke-ans').html(dataSet[y +  1])
  }
  else {
      $('#joke-qstn').html(dataSet[y - 1])
      $('#joke-ans').html(dataSet[y])
  }
  })
})